...
layui.define('jquery', function(exports){
    ...
    ,tag = function(){
      this.config = {
        likeHref:'../css/modules/tag/tag.css',
        skin: DEFAULT_SKIN,
        tagText:'+ New'
      };
      this.configs = {}
    ...
    var call = {
      ...
      ,add: function(e, tagElem, options){
        var  filter = tagElem.attr('lay-filter'),
            ...
            ,newTag = '<a href="javascript:;"  lay-id="'+ (options.text||'unnaming') +'"' +(options.attr ? ' lay-attr="'+ options.attr +'"' : '') +' type="button" class="' + TAG_ITEM  + '">'+ (options.text||'unnaming') +'</a>';
      ...
      ,tagAuto: function(filter){
        ...
        $(TAG_CLASS + elemFilter).each(function(){
          ...
          if(othis.attr('lay-newTag') && buttonNewTag.length === 0){
            buttonNewTag = $('<a href="javascript:;" type="button" class="' + BUTTON_NEW_TAG + '"></a>');
